...
When you start a `txt2img` generation, the extension captures all the current parameters. It converts them into a JSON structure identical to the standard WebUI API. It then modifies the `seed` to `-1` and sends a POST request to `[Target URL]/sdapi/v1/txt2img` in a background thread.
